...
namespace prog {
   ...
   // Image* aux = new Image(image->width(), image->height());
    ...
    for(int i = 0 ; i<w;i++){
        for(int j = 0; j < h ; j++){
            int nx_min = max(0,i - (ws/2));
            int nx_max = min(w - 1, i + (ws/2));
            int ny_min = max(0,j - (ws/2));
            int ny_max = min(h - 1, j + (ws/2));

            vector<rgb_value> red_values;
            vector<rgb_value> green_values;
            vector<rgb_value> blue_values;

            for(int fx = nx_min; fx <= nx_max; fx++){
                for(int fy = ny_min; fy <= ny_max; fy++){
                    red_values.push_back(image->at(fx,fy).red());
                    green_values.push_back(image->at(fx,fy).green());
                    blue_values.push_back(image->at(fx,fy).blue());
                }
            }
            ...
            int medianIndex = red_values.size() / 2 ;
            rgb_value medianR = red_values[medianIndex];
            rgb_value medianG = green_values[medianIndex];
            rgb_value medianB = blue_values[medianIndex];

            image->at(i,j).red() = medianR;
            image->at(i,j).green() = medianG;
            image->at(i,j).blue() = medianB;
